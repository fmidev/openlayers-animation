...
OpenLayers.Layer.Animation.LayerContainer = OpenLayers.Class({
    ...
    initialize : function(observer) {
         ...
         *
         * This is called when map is about to be dragged, panned or zoomed.
         *
        ...
        _mapEvents.movestart = function(event) {
            if (event) {
                ...
                // of loading ourselves. Notice, layers are set invisible here and
                // they are loaded in moveend function. Also notice, there may be
                // place for optimization here because some of the move functions
                // may not need visibility change when only part of tiles are updated.
                // But, current simple implementation uses callbacks consistently when
                // visibility of whole frame is changed and whole frame reloaded. This
                // implementation trusts that tile images are actually found from the
                // browser cache and do not need to be reloaded from the server.
                for (var i = 0; i < _layers.length; ++i) {
                    var layer = _layers[i];
                    // Prevent loading by setting layer visibility to false.
                    // Then, loading will be started when visibility is changed to true.
                    // Do not hide current frame. It should be shown until reloaded.
                    if (layer !== getCurrentFrame()) {
                        layer.setVisibility(false);
                        if (observer) {
                            // Inform observer because visibility change to false and
                            // back to visible will require reload of data. Notice,
                            // callback is not used here to inform current frame content
                            // as released because current frame content is kept visible
                            // until move operation ends.
         ...
         * Callback function for map event.
         *
         * This is called when map has been dragged, panned or zoomed.
         *
         * @param {Object} event OpenLayers event.
         */
        _mapEvents.moveend = function(event) {
            if (event) {
                var currentFrame = getCurrentFrame();
                if (currentFrame) {
                    // Hide current frame to make sure it is reloaded.
                    currentFrame.setVisibility(false);
                    if (observer) {
                        // Inform observer because visibility change to false and
                        // back to visible will require reload of data.
                        observer.frameContentReleasedCallback([currentFrame]);
                    }
                    // Current frame layer is loaded here to make sure it is the first
                    // one to be loaded and shown as soon as possible after it has been
                    // set invisible above.
                    currentFrame.loadLayer();
                }
